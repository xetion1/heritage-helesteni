...
export class RealTaskExecutor {
  ...
  async executeRealTask(task: DetailedTask): Promise<any> {
  ...
  private async executeResearchTask(task: DetailedTask): Promise<any> {
    ...
    await fs.writeFile(
      path.join(dataDir, `${task.id}.json`),
  ...
  private async executeSetupTask(task: DetailedTask): Promise<any> {
    if (task.command) {
  ...
  private async executeComponentTask(task: DetailedTask): Promise<any> {
    ...
    await fs.writeFile(
      path.join(componentDir, `${task.id}.test.tsx`),
      testContent
    );
  ...
  private async executePageTask(task: DetailedTask): Promise<any> {
  ...
  private async executeFeatureTask(task: DetailedTask): Promise<any> {
  ...
  private async executeTestTask(task: DetailedTask): Promise<any> {
    if (task.command) {
      try {
  ...
  private async executeOptimizationTask(task: DetailedTask): Promise<any> {
  ...
  private async executeVerificationTask(task: DetailedTask): Promise<any> {
    if (task.command) {
      try {
        const { stdout } = await execAsync(task.command, { cwd: this.projectRoot });
        return stdout;
  ...
  private async executeGenericTask(task: DetailedTask): Promise<any> {
    if (task.command) {
      const { stdout } = await execAsync(task.command, { cwd: this.projectRoot });
      return stdout;
    }
  ...
  private generateComponentCode(id: string, name: string): string {
...
`;
  ...
  private generateComponentStyles(id: string): string {
    return `.container {
  padding: 1rem;
  margin: 0 auto;
  max-width: 1200px;
...
`;
  ...
  private generateComponentTest(id: string): string {
...
`;
  ...
  private generatePageCode(id: string, name: string): string {
    return `import React from 'react';
...
`;
  ...
  private generatePageMetadata(id: string, name: string): string {
...
export function generateAllTasks(): DetailedTask[] {
  const tasks: DetailedTask[] = [];
  
  // RESEARCH PHASE (15 tasks)
  tasks.push(
    { id: 'research-area', category: 'research', name: 'Research Heleșteni area and tourism' },
    { id: 'research-demographics', category: 'research', name: 'Analyze visitor demographics' },
    { id: 'research-competitors', category: 'research', name: 'Study competitor heritage sites' },
    { id: 'research-tech', category: 'research', name: 'Research latest tech stack' },
    { id: 'research-seo', category: 'research', name: 'SEO keyword research' },
    { id: 'research-accessibility', category: 'research', name: 'Accessibility standards research' },
    { id: 'research-booking', category: 'research', name: 'Booking system requirements' },
    { id: 'research-payment', category: 'research', name: 'Payment gateway options' },
    { id: 'research-multilang', category: 'research', name: 'Multi-language requirements' },
    { id: 'research-media', category: 'research', name: 'Media optimization strategies' },
    { id: 'research-social', category: 'research', name: 'Social media integration' },
    { id: 'research-analytics', category: 'research', name: 'Analytics implementation' },
    { id: 'research-security', category: 'research', name: 'Security best practices' },
    { id: 'research-performance', category: 'research', name: 'Performance benchmarks' },
    { id: 'research-hosting', category: 'research', name: 'Hosting and deployment options' }
  );
  
  // SETUP PHASE (25 tasks)
  tasks.push(
    { id: 'setup-git', category: 'setup', name: 'Initialize git repository', command: 'git init' },
    { id: 'setup-npm', category: 'setup', name: 'Initialize package.json', command: 'npm init -y' },
    { id: 'setup-next', category: 'setup', name: 'Install Next.js', command: 'npm install next@latest react@latest react-dom@latest' },
    { id: 'setup-typescript', category: 'setup', name: 'Install TypeScript', command: 'npm install -D typescript @types/react @types/node' },
    { id: 'setup-tailwind', category: 'setup', name: 'Install Tailwind CSS', command: 'npm install -D tailwindcss postcss autoprefixer' },
    { id: 'setup-eslint', category: 'setup', name: 'Install ESLint', command: 'npm install -D eslint eslint-config-next' },
    { id: 'setup-prettier', category: 'setup', name: 'Install Prettier', command: 'npm install -D prettier' },
    { id: 'setup-testing', category: 'setup', name: 'Install testing libraries', command: 'npm install -D jest @testing-library/react @testing-library/jest-dom' },
    { id: 'setup-playwright', category: 'setup', name: 'Install Playwright', command: 'npm install -D @playwright/test' },
    { id: 'setup-prisma', category: 'setup', name: 'Install Prisma', command: 'npm install prisma @prisma/client' },
    { id: 'setup-auth', category: 'setup', name: 'Install NextAuth', command: 'npm install next-auth' },
    { id: 'setup-forms', category: 'setup', name: 'Install React Hook Form', command: 'npm install react-hook-form' },
    { id: 'setup-validation', category: 'setup', name: 'Install Zod', command: 'npm install zod' },
    { id: 'setup-ui', category: 'setup', name: 'Install UI libraries', command: 'npm install @radix-ui/react-dialog @radix-ui/react-dropdown-menu' },
    { id: 'setup-icons', category: 'setup', name: 'Install Lucide icons', command: 'npm install lucide-react' },
    { id: 'setup-animation', category: 'setup', name: 'Install Framer Motion', command: 'npm install framer-motion' },
    { id: 'setup-maps', category: 'setup', name: 'Install map libraries', command: 'npm install leaflet react-leaflet' },
    { id: 'setup-calendar', category: 'setup', name: 'Install calendar library', command: 'npm install react-big-calendar' },
    { id: 'setup-carousel', category: 'setup', name: 'Install carousel library', command: 'npm install embla-carousel-react' },
    { id: 'setup-toast', category: 'setup', name: 'Install toast notifications', command: 'npm install react-hot-toast' },
    { id: 'setup-swr', category: 'setup', name: 'Install SWR for data fetching', command: 'npm install swr' },
    { id: 'setup-i18n', category: 'setup', name: 'Install i18n library', command: 'npm install next-i18next' },
    { id: 'setup-seo', category: 'setup', name: 'Install SEO library', command: 'npm install next-seo' },
    { id: 'setup-analytics', category: 'setup', name: 'Install analytics', command: 'npm install @vercel/analytics' },
    { id: 'setup-monitoring', category: 'setup', name: 'Install monitoring', command: 'npm install @sentry/nextjs' }
  );
